...
export default function WallpaperClientApp({
  ...
  const [materialized, setMaterialized] =
    useState<MaterializedPlayerResources>({
      ...bundledResources,
      audios: [...bundledResources.audios],
  ...
  const persistence = useMemo(
    () =>
      createPlayerPersistence({
        database,
        resources: {
          audios: materialized.audios.map((item) => item.id),
    ...
    <section className="grid min-h-dvh place-items-center bg-muted/30 p-6">
      <Card className="w-full max-w-lg">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Open Resources, then add at least one model, motion, audio
            track, and stage. Everything you add is cached only on this
            device.
          </CardDescription>
        </CardHeader>
      </Card>
