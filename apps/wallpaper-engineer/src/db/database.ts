...
export const CLIENT_DB_V8_STORES = {
  meta: "key, updatedAt",
  settings: "key, updatedAt",
  resources:
    "id, sourceId, [sourceId+upstreamId], kind, visibility, updatedAt, *tags",
  resourceVersions:
    "id, sourceId, [sourceId+upstreamId], resourceId, sha256, publishedAt",
  resourceDependencies:
    "[parentVersionId+binding], parentVersionId, dependencyVersionId",
  libraryItems:
    "resourceId, [resourceId+resourceVersionId], resourceVersionId, sourceId, kind, source, addedAt",
  playlists: "id, updatedAt",
  playlistItems:
    "id, playlistId, [playlistId+position], modelId, motionId, stageId",
  artifactMetadata:
...
export class WallpaperClientDatabase extends Dexie {
  ...
  resourceDependencies!: Table<ResourceDependencyRecord, [string, string]>;
  ...
  constructor(
    ...
    this.version(8)
      .stores(CLIENT_DB_V8_STORES)
      .upgrade(async (transaction: Transaction) => {
        await Promise.all([
          transaction.table("resources").clear(),
          transaction.table("resourceVersions").clear(),
          transaction.table("resourceDependencies").clear(),
          transaction.table("libraryItems").clear(),
          transaction.table("artifactMetadata").clear(),
          transaction.table("playlists").clear(),
          transaction.table("playlistItems").clear(),
