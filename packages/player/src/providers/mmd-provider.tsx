  ...
  return models.flatMap((_, modelIndex) =>
    motions.map((__, motionIndex) => ({
      ...
      audioId:
        audios.find(
          (audio) =>
            audio.id === `${motions[motionIndex].id}:audio`,
        )?.id ?? audios[motionIndex % audios.length].id,
      stageId: stages[0].id,
    })),
  );
}
...
function normalizePlaylist(
  ...
  if (!Array.isArray(value)) {
    return createDefaultPlaylist(models, motions, audios, stages);
  }
  ...
  const normalized = value.flatMap((item, index): MmdPlaylistItem[] => {
    ...
    const audioId =
      typeof candidate.audioId === "string"
        ? candidate.audioId
        : Number.isInteger(candidate.audioIndex)
          ? audios[candidate.audioIndex!]?.id
          : legacyAudioId !== undefined &&
              audioIds.has(legacyAudioId)
            ? legacyAudioId
            : audios[0].id;
    if (
      modelId === undefined ||
  ...
  return normalized.length > 0
    ? normalized
    : createDefaultPlaylist(models, motions, audios, stages);
}
...
function getResourceKey(
  ...
  const audio = audios.find(
    (candidate) => candidate.id === item.audioId,
  )!;
  const stage = stages.find((candidate) => candidate.id === item.stageId)!;
  return `${item.id}|${model.modelPath}|${stage.stagePath ?? "solid-color"}|${audio.audioPath}|${motion.motionPath.join("|")}|camera-delay-${motion.cameraDelaySeconds ?? 0}`;
}
...
export function MmdProvider({
  ...
  if (
    ...
    throw new Error(
      "MmdProvider requires at least one model, motion, audio, and stage.",
    );
  }
  ...
  const playlist = useMemo(
    ...
    [storedPlaylist, models, motions, audios, stages],
  );
  ...
  const audioIndex = audios.findIndex(
    (candidate) => candidate.id === playlistItem.audioId,
  );
  ...
  const setPlaylist = useCallback(
    (value: SetStateAction<MmdPlaylistItem[]>): void => {
      ...
      setStoredPlaylist((current) => {
        ...
        return normalizePlaylist(next, models, motions, audios, stages);
      });
    },
    [models, motions, audios, stages, markPersistenceDirty],
  );
  ...
  const loadSlot = useCallback(
    (
      ...
      const promise = targetController
        .load(canvas, {
          ...
          audioPath: targetAudio.audioPath,
    ...
    [canvases, controllers, models, motions, audios, stages],
  );
  ...
  const updateCurrentItem = useCallback(
    (
      update: Partial<
        Pick<
          MmdPlaylistItem,
          "modelId" | "motionId" | "audioId" | "stageId"
        >
      >,
    ) => {
      setPlaylist((current) =>
  ...
  const selectAudio = useCallback(
    (index: number): void => {
      assertIndex(index, audios.length, "audio");
      updateCurrentItem({ audioId: audios[index].id });
    },
    [audios, updateCurrentItem],
  );
  ...
  const previousAudio = useCallback(() => {
    selectAudio(wrapIndex(audioIndex - 1, audios.length));
  }, [audioIndex, audios.length, selectAudio]);
