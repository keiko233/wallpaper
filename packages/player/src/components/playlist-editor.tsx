...
export function PlaylistEditor() {
  const { models, motions, audios, stages, playlist, playlistIndex } =
    useMmdState();
    ...
    <Dialog>
      ...
      <DialogPopup className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Combine any model, motion, audio, and stage. The list loops
            continuously and is saved automatically on this device.
          </DialogDescription>
        </DialogHeader>
            ...
            {playlist.map((item, index) => (
              ...
              >
                ...
                <Select
                  onValueChange={(value) => {
                    if (value !== null) {
                      updateItem(index, { audioId: value });
                    }
                  }}
                  value={item.audioId}
                >
                  <SelectTrigger aria-label={`Audio for item ${index + 1}`}>
                    <SelectValue>
                      {
                        audios.find((audio) => audio.id === item.audioId)!
                          .name
                      }
                    </SelectValue>
                  </SelectTrigger>
          ...
          <div className="rounded-xl border border-dashed p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  onValueChange={(value) => {
                    if (value !== null) setAudioIndex(Number(value));
                  }}
                  value={String(audioIndex)}
                >
                  <SelectTrigger>
                    <SelectValue>{audios[audioIndex].name}</SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {audios.map((audio, index) => (
                      <SelectItem key={audio.id} value={String(index)}>
                        {audio.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
